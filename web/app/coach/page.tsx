...
import { Brain, Scale, Utensils, Dumbbell, BarChart2, Target, TrendingDown, Zap } from 'lucide-react';
...
function buildSystem(store: ReturnType<typeof useStore>): string {
  ...
  const avgHabit = store.habits.length
    ? store.habits.map(h => { const days = Array.from({ length: 7 }, (_, i) => { const d = new Date(); d.setDate(d.getDate() - i); return d.toISOString().slice(0, 10); }); return days.filter(d => h.completions.includes(d)).length / 7; }).reduce((a, b) => a + b, 0) / store.habits.length
    : 0;
...
export default function CoachPage() {
  ...
  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [store.coachMessages, loading]);
  ...
  async function sendMessage(text?: string) {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/claude', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ system: buildSystem(store), messages: [...store.coachMessages.slice(-10).map(m => ({ role: m.role === 'coach' ? 'assistant' : 'user', content: m.content })), { role: 'user', content: msg }], userApiKey: store.settings.anthropicApiKey }) });
      const data = await res.json();
      store.addCoachMessage({ id: nanoid(), role: 'coach', content: data.text ?? data.error ?? 'No response. Keep pushing.', timestamp: new Date().toISOString() });
    } catch {
  ...
  const promptIcons = [BarChart2, Target, TrendingDown, Zap];
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] lg:h-[calc(100vh-48px)]">
      ...
      {store.coachMessages.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-6 pb-4">
            ...
            {checkinPrompts.map((p, i) => {
              ...
              return (
                <button key={i} onClick={() => sendMessage(p)}
                  className="w-full text-left px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-700 hover:border-blue-300 hover:bg-blue-50 transition-all font-medium flex items-start gap-2">
                  <PromptIcon size={16} className="flex-shrink-0 mt-0.5 text-gray-400" />
          ...
          {!store.settings.anthropicApiKey && !process.env.ANTHROPIC_API_KEY && (
            <div className="text-center text-sm text-orange-600 bg-orange-50 px-4 py-3 rounded-xl max-w-sm">
              Add your Anthropic API key in <strong>Settings</strong> to enable the AI Coach
            </div>
          )}
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto space-y-4 pb-4">
          {store.coachMessages.map(msg => (
            <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              {msg.role === 'coach' && (
                <div className="w-9 h-9 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 mt-auto">
                  <Brain size={16} className="text-white" />
                </div>
              )}
              <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-white border border-gray-100 text-gray-800 rounded-bl-sm shadow-sm'
              }`}>{msg.content}</div>
            </div>
          ))}
          {loading && (
      ...
      <div className="flex-shrink-0 flex gap-3 pt-3 border-t border-gray-100 bg-[#F5F5F7]">
        <textarea ref={inputRef} className="flex-1 input resize-none min-h-[44px] max-h-32 py-3" placeholder="Ask your coach anything..." rows={1} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} />
        <button onClick={() => sendMessage()} disabled={!input.trim() || loading} className="w-11 h-11 bg-blue-600 text-white rounded-xl flex items-center justify-center disabled:opacity-40 hover:bg-blue-700 transition-colors flex-shrink-0 self-end">↑</button>
      </div>
    </div>
  );
}
