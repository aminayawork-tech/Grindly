      ...
      {store.coachMessages.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-6 pb-4">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {[
              { emoji: '⚖️', value: `${trend.current.toFixed(1)} lbs` },
              { emoji: '🍽️', value: `${Math.round(store.todayCalories())} kcal today` },
              { emoji: '🏋️', value: `${store.weeklyWorkoutCount()} workouts/wk` },
            ].map(chip => (
              <div key={chip.value} className="flex items-center gap-1.5 bg-gray-100 text-gray-700 text-sm font-medium px-3 py-1.5 rounded-full">
                <span>{chip.emoji}</span>{chip.value}
              </div>
            ))}
          </div>
          ...
          <div className="w-full max-w-sm space-y-2">
            {checkinPrompts.map((p, i) => (
              <button key={i} onClick={() => sendMessage(p)}
                className="w-full text-left px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-700 hover:border-blue-300 hover:bg-blue-50 transition-all font-medium">
                {i === 0 ? '📊' : i === 1 ? '🎯' : i === 2 ? '📉' : '⚡'} {p.length > 60 ? p.slice(0, 60) + '...' : p}
              </button>
            ))}
          </div>
          ...
          {!store.settings.anthropicApiKey && !process.env.ANTHROPIC_API_KEY && (
            <div className="text-center text-sm text-orange-600 bg-orange-50 px-4 py-3 rounded-xl max-w-sm">
              ⚠️ Add your Anthropic API key in <strong>Settings</strong> to enable the AI Coach
            </div>
          )}
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto space-y-4 pb-4">
          {store.coachMessages.map(msg => (
            <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              {msg.role === 'coach' && (
                <div className="w-9 h-9 bg-blue-600 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0 mt-auto">🧠</div>
              )}
          ...
          {loading && (
            <div className="flex gap-3">
              <div className="w-9 h-9 bg-blue-600 rounded-full flex items-center justify-center text-white text-sm flex-shrink-0">🧠</div>
