...
import { Dumbbell, Utensils, Scale, CheckSquare, Flame, TrendingUp, Target, Brain } from 'lucide-react';
...
function ScoreIcon({ score }: { score: number }) {
  if (score >= 80) return <Flame size={48} className="text-orange-500" />;
  if (score >= 60) return <Dumbbell size={48} className="text-blue-500" />;
  if (score >= 40) return <TrendingUp size={48} className="text-green-500" />;
  return <Target size={48} className="text-gray-400" />;
}
...
export default function ReportPage() {
  ...
  async function getCoachTake() {
    ...
    try {
      const res = await fetch('/api/claude', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ system: `You are a direct, no-BS life coach reviewing someone's weekly health and fitness data from the Grindly app. Be honest, specific, and actionable. Max 4 sentences.`, messages: [{ role: 'user', content: prompt }], userApiKey: store.settings.anthropicApiKey }) });
      const data = await res.json();
      setCoachMessage(data.text ?? data.error ?? 'Coach unavailable.');
    } catch { setCoachMessage('Coach is unavailable right now.'); }
    setLoadingCoach(false);
  }

  const weekRange = (() => { const end = new Date(); const start = new Date(); start.setDate(start.getDate() - 6); return `${start.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – ${end.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`; })();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card bg-gradient-to-br from-blue-50 to-purple-50 border-0 text-center py-8">
        <div className="flex justify-center mb-2"><ScoreIcon score={score} /></div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <StatCard label="Weight Change" value={`${weightChange >= 0 ? '+' : ''}${weightChange.toFixed(1)} lbs`} subtitle={weightChange < 0 ? 'Great progress!' : 'Watch the trend'} icon={<Scale size={14} />} color={weightChange <= 0 ? 'blue' : 'orange'} compact />
            ...
            {store.habits.map(habit => {
              ...
              return (
                <div key={habit.id} className="flex items-center gap-3">
                  <div className="w-7 flex-shrink-0 flex items-center justify-center text-gray-500">
                  ...
                  <div className="flex gap-0.5">
                    {days.map(d => (
                      <div key={d} title={d} className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${habit.completions.includes(d) ? `${colorCls.bg} text-white` : 'bg-gray-100 text-gray-300'}`}>
      ...
      <div className="card">
        ...
        {coachMessage ? (
          <div className="flex gap-3">
            <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0"><Brain size={18} className="text-white" /></div>
            <div className="bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-800 leading-relaxed flex-1">{coachMessage}</div>
          </div>
        ) : loadingCoach ? (
          <div className="flex items-center gap-3 text-gray-400">
            <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center"><Brain size={18} className="text-white" /></div>
