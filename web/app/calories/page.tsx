    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <div className="flex gap-3 mt-4">
          {[{ label: 'Protein', value: todayProtein, color: 'bg-blue-100 text-blue-700', unit: 'g' }, { label: 'Carbs', value: todayCarbs, color: 'bg-orange-100 text-orange-700', unit: 'g' }, { label: 'Fat', value: todayFat, color: 'bg-yellow-100 text-yellow-700', unit: 'g' }].map(m => (
      ...
      <div className="card-sm flex items-center gap-4">
        ...
        <div className="text-right">
          <div className={`text-sm font-semibold ${sevenAvg > goal ? 'text-red-500' : 'text-green-600'}`}>{sevenAvg > goal ? `+${Math.round(sevenAvg - goal)} over` : `${Math.round(goal - sevenAvg)} under`}</div>
            ...
            <div key={meal.id} className="card">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <meal.Icon size={18} className="text-gray-500" />
                ...
                <button onClick={() => { setSelectedMeal(meal.id); setShowSearch(true); }} className="text-blue-600 font-semibold text-sm hover:text-blue-700">+ Add</button>
              </div>
              {entries.length === 0 ? (
                ...
                <div className="space-y-2">
                  {entries.map(e => (
                    <div key={e.id} className="flex items-center gap-3">
                      ...
                      <button onClick={() => store.removeFoodEntry(e.id)} className="text-red-300 hover:text-red-500 flex-shrink-0"><X size={14} /></button>
                    </div>
                  ))}
                </div>
              )}
